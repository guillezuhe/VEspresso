...
where $\Kappa_m$ and $\tau_m$ are fitting parameters. We will discuss in following sections how to obtain these fitting parameters. We can now define an extended variable $Z_{i,m} (t)$:
...
with $\zeta_m = \tau_m \Kappa_m$. If $W_{i,m}$ is a standard Wiener process, this SDE corresponds to an Ornstein-Uhlenbeck (OU) process. Using the OU properties, one can see that:
...
2. Force computation:
   $$
   F_i^{n+1} = \left( F_i^c(x^n) + \sqrt{\frac{2 k_B T \zeta}{\Delta t}} B_{i,N}^{n} + \sum_{m=1}^{N_m}{\Kappa_m U_{i,m}^{n+1}} \right)
   $$
3. Position update
   $$
